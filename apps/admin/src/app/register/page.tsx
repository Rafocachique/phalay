...
import { adminRegister } from '@/app/auth/actions';
import { useActionState } from 'react';
import Link from 'next/link';

export default function AdminRegisterPage() {
  const [state, formAction, isPending] = useActionState(adminRegister, null);
        ...
        <div className="grid md:grid-cols-2 gap-0 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-sm">
          ...
          <div className="hidden md:flex flex-col justify-between p-12 bg-gradient-to-br from-[#8B5A5A] to-[#A87474] text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="text-3xl lg:text-4xl font-serif font-bold mb-4 tracking-tight leading-tight">Nuevo Admin</h2>
              <p className="text-white/85 text-sm leading-relaxed">
                Crea tu cuenta de administrador de PHALAY para gestionar la tienda.
              </p>
            </div>
            ...
            <div className="relative z-10 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <span className="text-sm text-white/95">Acceso total al catálogo y colecciones</span>
              </div>
              <div className="flex items-start gap-3">
                <svg className="w-5 h-5 text-white/90 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                <span className="text-sm text-white/95">Gestión y control de inventario de variantes</span>
              </div>
              <div className="flex items-start gap-3">
                <svg className="w-5 h-5 text-white/90 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                <span className="text-sm text-white/95">Monitoreo de pedidos y envíos</span>
              </div>
            </div>
          ...
          <div className="bg-white p-8 md:p-12 flex flex-col justify-center">
            <div className="mb-6">
              <h1 className="text-2xl font-serif font-bold text-gray-900 mb-2">Crear Administrador</h1>
              <p className="text-sm text-gray-600">Registra tus datos de acceso rápido</p>
            </div>
            ...
            <form action={formAction} className="space-y-4 mb-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    disabled={isPending}
                    placeholder="Elena"
                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:bg-white focus:border-[#8B5A5A] focus:ring-2 focus:ring-[#8B5A5A]/10 outline-none transition-all"
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <input
                  ...
                  required
                  disabled={isPending}
                  placeholder="ejemplo@phalay.com"
                  className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder:text-gray-400 focus:bg-white focus:border-[#8B5A5A] focus:ring-2 focus:ring-[#8B5A5A]/10 outline-none transition-all"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={isPending}
                className="w-full bg-gradient-to-r from-[#8B5A5A] to-[#A87474] hover:from-[#9B6A6A] hover:to-[#B88484] text-white font-bold py-3.5 rounded-xl transition-all transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg"
              >
                {isPending && (
            ...
            <div className="text-center text-xs text-gray-500">
              <p>¿Ya tienes cuenta?{' '}
                <Link href="/login" className="text-[#8B5A5A] font-semibold hover:underline">
                  Inicia sesión aquí
                </Link>
              </p>
            </div>
